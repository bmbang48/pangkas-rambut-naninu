...
<header class="mb-12" data-aos="fade-up">
    ...
    <h1 class="text-4xl font-bold tracking-tight">Edit <span class="text-primary">Barber</span></h1>
</header>

<div class="max-w-2xl" data-aos="fade-up" data-aos-delay="100">
    <div class="glass rounded-3xl p-8 border-white/5 shadow-2x-over">
        <form action="{{ route('barbers.update', $barber->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            
            <div>
                <label for="name" class="block text-sm font-bold text-gray-400 uppercase tracking-widest mb-2">Nama Lengkap</label>
                <input type="text" name="name" id="name" value="{{ old('name', $barber->name) }}" required
            ...
            <div>
                ...
                <input type="number" name="experience" id="experience" value="{{ old('experience', $barber->experience) }}" required
                    class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-primary/50 transition-colors"
                    placeholder="misal: 5">
                @error('experience')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            ...
            <div>
                ...
                <div class="flex items-center gap-4">
                    <div id="image-preview-container">
                        @if($barber->photo)
                            <img id="image-preview" src="{{ asset('storage/barbers/' . $barber->photo) }}" alt="Preview" class="w-20 h-20 rounded-xl object-cover border border-white/10">
                        @else
                            <img id="image-preview" src="#" alt="Preview" class="w-20 h-20 rounded-xl object-cover border border-white/10 hidden">
                        @endif
                    </div>
                    <div class="flex-1">
                        ...
                        <p class="text-gray-500 text-xs mt-2">Biarkan kosong untuk mempertahankan foto saat ini. Rekomendasi: Kotak, maks 2MB.</p>
                    </div>
                </div>
                @error('photo')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
